...
if input_mode == "Ползунки":
    ccu = st.sidebar.slider("Средний онлайн (CCU):", 10, 50000, value=500, step=50, key="ccu_s")
    session_time = st.sidebar.slider("Длина сессии (минут):", 1, 120, value=15, step=1, key="sess_s")
    
    # Фиксированный D1 напрямую от пользователя
    d1_input = st.sidebar.slider("D1 Retention (%):", 10.0, 75.0, value=32.0, step=1.0, key="d1_s")
    ...
    base_conv = st.sidebar.slider("Конверсия доната (%):", 0.5, 10.0, value=2.5, step=0.1, key="conv_s") / 100.0
    base_arppu = st.sidebar.slider("Средний чек (R$):", 50, 2000, value=280, step=10, key="arppu_s")

    st.sidebar.markdown("---")
    
    # БЛОК НАСТРОЕК CREATOR REWARDS
    st.sidebar.subheader("💎 Creator Rewards")
    vgu_ratio = st.sidebar.slider("Доля Active Spenders на платформе (%):", 0.5, 15.0, value=7.0, step=0.5, key="vgu_s") / 100.0
    behavioral_filter = st.sidebar.slider("Эффективность фильтра (10+ мин) (%):", 1.0, 50.0, value=12.0, step=0.5, key="filt_s") / 100.0
    ae_percent = st.sidebar.slider("Audience Expansion :", 0.1, 5.0, value=1.0, step=0.1, key="ae_s") / 100.0
    
    st.sidebar.markdown("---")
    with st.sidebar.container():
        st.subheader("💰 Реинвестирование")
        devex_rate = st.sidebar.slider("Курс DevEx ($ за 1 R$):", 0.0010, 0.0100, value=0.0035, step=0.0001, format="%.4f", key="dev_s")
        tax_rate = st.sidebar.slider("Налог на вывод (%):", 0, 20, value=6, step=1, key="tax_s") / 100.0
        reinvest_rate = st.sidebar.slider("Поддержка игры / Фонд развития (%):", 0, 50, value=15, step=5, key="rei_s") / 100.0
        marketing_rate = st.sidebar.slider("Маркетинг (%):", 0, 40, value=10, step=5, key="mkt_s") / 100.0
        share = st.sidebar.slider("Доля инвестора (%):", 0, 100, value=35, step=5, key="shr_s") / 100.0
else:
    ccu = st.sidebar.number_input("Средний онлайн (CCU):", 0, 100000, value=500, step=100, key="ccu_n")
    session_time = st.sidebar.number_input("Длина сессии (мин):", 1, 240, value=15, step=1, key="sess_n")
    
    d1_input = st.sidebar.number_input("Day 1 Retention (%):", 0.0, 100.0, value=32.0, step=1.0, key="d1_n")
    ...
    st.sidebar.subheader("💸 Монетизация")
    base_conv = st.sidebar.number_input("Конверсия (%):", 0.0, 100.0, value=2.5, step=0.1, key="conv_n") / 100.0
    base_arppu = st.sidebar.number_input("Средний чек (R$):", 0, 100000, value=280, step=50, key="arppu_n")
    
    st.sidebar.subheader("💎 Creator Rewards")
    vgu_ratio = st.sidebar.number_input("Доля Active Spenders (%):", 0.0, 100.0, value=7.0, step=0.5, key="vgu_n") / 100.0
    behavioral_filter = st.sidebar.number_input("Эффективность фильтра (%):", 0.0, 100.0, value=12.0, step=0.5, key="filt_n") / 100.0
    ae_percent = st.sidebar.number_input("Audience Expansion :", 0.1, 5.0, value=1.0, step=0.1, key="ae_n") / 100.0
    
    st.sidebar.markdown("---")
    with st.sidebar.container():
        st.subheader("💰 Налоги, Курс and Распределение")
        devex_rate = st.sidebar.number_input("Курс DevEx ($ за 1 R$):", 0.0000, 0.0100, value=0.0035, step=0.0001, format="%.4f", key="dev_n")
        tax_rate = st.sidebar.number_input("Налог на вывод (%):", 0, 100, value=6, step=1, key="tax_n") / 100.0
        reinvest_rate = st.sidebar.number_input("Поддержка игры / Фонд развития (%):", 0, 100, value=15, step=5, key="rei_n") / 100.0
        marketing_rate = st.sidebar.number_input("Маркетинг (%):", 0, 100, value=10, step=5, key="mkt_n") / 100.0
        share = st.sidebar.number_input("Доля инвестора (%):", 0, 100, value=35, step=5, key="shr_n") / 100.0
...
with fig_col1:
    ...
    ax1.plot(range(1, 31), [d1] + [d1 * (t**-alpha) for t in range(2, 31)], color='#7d3cff', linewidth=3)
    st.pyplot(fig1)
...
with fig_col2:
    ...
    ccu_levels = [100, 500, 1000, 2500, 5000]
    ax2.bar(['100', '500', '1к', '2.5к', '5к'], [c * 0.015 for c in ccu_levels], color='#ffb703')
    st.pyplot(fig2)
...
balance = -INVESTMENT + (investor_payout_usd * months)
ax3.plot(months, balance, color='#00f2ff', marker='o', linewidth=3)
