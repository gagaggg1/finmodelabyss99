...
st.write("Профессиональный расчет доходности с учетом маркетингового бюджета и реинвестиций.")
...
if "marketing_rate" not in st.session_state: st.session_state["marketing_rate"] = 10  # Новый параметр!
...
# 1. Автоматический расчет MAU и CCU
...
# Экономика расходов и выплат (Включая Маркетинг)
...
# Чистая прибыль теперь вычитает и налоги, и фонд обновлений, и маркетинг
...
sys_col1.metric("Средний онлайн (CCU)", f"{ccu:,} чел.")
sys_col2.metric("Аудитория за месяц (MAU)", f"{mau:,} чел.")
...
col1, col2, col3, col4, col5 = st.columns(5) # Добавили 5-ю колонку для Маркетинга
...
m_col2.metric("Sticky Factor", f"{sticky_factor_pct:.1f}%")
