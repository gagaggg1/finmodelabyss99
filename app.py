...
# НОВАЯ ЛОГИКА: Натуральный MAU и Приток
retention_list = [1.0 if t == 0 else d1 * (t ** -alpha) for t in range(30)]
mau = dau * sum(retention_list) / (1 + sum([d1 * (t ** -alpha) for t in range(1, 30)]))
...
monthly_paying_users = (dau * real_conv) * effective_lifetime
...
# --- ГРАФИК ---
st.markdown("---")
st.subheader("📉 Динамика возврата инвестиций")
fig, ax = plt.subplots(figsize=(10, 3.5))
months = np.arange(0, 7)
ax.plot(months, -INVESTMENT + (investor_payout_usd * months), color='#00ff41', marker='o')
