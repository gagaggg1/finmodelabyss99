...
# --- ИНТЕРФЕЙС: БОКОВАЯ ПАНЕЛЬ (ТОЛЬКО ИГРОВЫЕ МЕТРИКИ) ---
...
# --- ВЫДЕЛЕННАЯ ПЛАШКА ДЛЯ НАЛОГОВ И РАСПРЕДЕЛЕНИЯ ПРИБЫЛИ ---
with st.expander("💰 НАЛОГИ, ФОНДЫ И ДОЛИ (НАСТРОЙКА РАСПРЕДЕЛЕНИЯ)", expanded=True):
    p_col1, p_col2, p_col3, p_col4 = st.columns(4)
    
    with p_col1:
        tax_rate = st.number_input("Налог на вывод (%):", 0, 100, value=6, step=1) / 100.0
    with p_col2:
        reinvest_rate = st.number_input("Фонд развития (%):", 0, 100, value=15, step=5) / 100.0
    with p_col3:
        marketing_rate = st.number_input("Маркетинг (%):", 0, 100, value=10, step=5) / 100.0
    with p_col4:
        share = st.number_input("Доля инвестора (%):", 0, 100, value=35, step=5) / 100.0
...
# Трафик фиксирован относительно TARGET_SESSION, чтобы длина сессии не резала базу игроков
...
# Премиум-выплаты растут от наигранных минут
...
# Отрисовка линии с неоновым эффектом
ax.plot(timeline, balance_timeline, color='#00ffcc', marker='o', linewidth=2.5, zorder=3, label="Баланс инвестора ($)")
ax.plot(timeline, balance_timeline, color='#00ffcc', alpha=0.3, linewidth=6, zorder=2) # Свечение
