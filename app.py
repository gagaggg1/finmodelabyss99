...
if input_mode == "Ползунки":
    ...
    st.sidebar.subheader("💸 Монетизация")
    base_conv = st.sidebar.number_input("Конверсия доната (%):", 0.0, 10.0, value=2.5, step=0.1) / 100.0
    base_arppu = st.sidebar.number_input("Средний чек (R$):", 0, 100000, value=280, step=10)
